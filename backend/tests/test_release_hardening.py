...
def test_release_docs_and_recipe_matrix_exist() -> None:
    ...
    for relative in [
        ...
        "release_evidence/v0.9.5-rc.1/RC_FREEZE_REPORT.redacted.json",
    ...
    assert "v0.9.5 RC Freeze Evidence" in summary
    assert "RC PASS" in (root / "release_candidate_report.md").read_text(encoding="utf-8")
...
def test_release_scripts_exist_and_electron_high_fix_is_pinned() -> None:
    ...
    assert root_package["version"] == "0.9.5-rc.1"
    assert package["version"] == "0.9.5-rc.1"
    assert '__version__ = "0.9.5-rc.1"' in backend_init
    assert package["devDependencies"]["electron"] == "42.5.0"


def test_release_evidence_is_redacted() -> None:
    root = project_root()
    evidence_dir = root / "release_evidence/v0.9.5-rc.1"
